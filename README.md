...
* Worked from the provided 8 rows before generating the full 200 row seed dataset, so the normaliser backend service `IncidentReviewer/Services/IncidentNormalizer.cs` was validated against real cases first.

## One decision I would make differently with more time

* Add file-based batch import (e.g. CSV to JSON conversion). The batch endpoint currently expects JSON, which adds a step for operators working from spreadsheets. The right format depends on what the end user uses, so I left it open rather than assume something.

## One thing I deliberately left out and why

* A frontend. Different teams or organisations may want the data presented differently such as through a web dashboard, mobile app, or reporting tool. A clean API leaves that decision open and avoids coupling the backend to assumptions made under time pressure.

## What I noticed about the data and how I handled it

* The source data had inconsistent formats. The normaliser handles these at ingest so the database holds consistently formatted records, supporting readability and any future analysis.
